...
import { getDemoSales } from '../services/demo';
...
const Sales: React.FC = () => {
    ...
    const filteredSales = sales.filter((sale) => {
        const customerName = sale.customers
            ? `${sale.customers.first_name} ${sale.customers.last_name}`.toLowerCase()
            : '';
        const vehicleName = sale.vehicles
            ? `${sale.vehicles.brand} ${sale.vehicles.model}`.toLowerCase()
            : '';
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div className="card bg-blue-50 border-blue-100">
                    <p className="text-sm font-medium text-blue-600">Toplam Satış Adedi</p>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {filteredSales.map((sale) => (
                                <tr key={sale.id} className="hover:bg-gray-50 cursor-pointer">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className={`badge-${sale.status === 'completed' ? 'success' :
                                            sale.status === 'pending' ? 'warning' : 'danger'
                                            }`}>
                                            {sale.status === 'completed' ? 'Tamamlandı' :
                                                sale.status === 'pending' ? 'Bekliyor' : 'İptal'}
                                        </span>
                                    </td>
                                </tr>
                            ))}
